[CmdletBinding()]
param(
    [string] $FeedPath = $env:ATYA_LOCAL_NUGET_FEED,
...
if ([string]::IsNullOrWhiteSpace($FeedPath)) {
    $FeedPath = Join-Path $HOME ".nuget/local-atya"
}
...
function Get-SolutionFile {
    ...
    $solutionFiles = @(Get-ChildItem -LiteralPath $RootPath -Filter "*.sln" -File)
...
$solutionFile = Get-SolutionFile -RootPath $repoRoot

if (-not $solutionFile) {
    $parentRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path

    $parentSolutionFile = Get-SolutionFile -RootPath $parentRoot
    ...
    if ($parentSolutionFile) {
        ...
        throw "Unable to locate a single solution file from script path '$PSScriptRoot'."
    }
}
